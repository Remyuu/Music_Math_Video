...
#2-zoom camera
        ...
        D_set = [] ;vg = VGroup()
        for value in range(430):
            position = new_axes.i2gp(graph=graph_snippet,x=(value-1)/100)
            dot = Dot(color=YELLOW,radius=0.02)
        ...
        new_axes, graph_snippet = self.show_zoom_camera(axes=new_axes,graph=vg,zoom_start=0.5,zoom_rect_dims=(0.1, 5.0))
